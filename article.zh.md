...
## GPT‑6 Astra、Blender 与可执行三维世界的一次可复核工程实验
...
<p class="table-heading" id="table-2-title"><span class="table-number">Table 2.</span> 四种建模方法与三种融合基线的相机位姿和深度误差</p>
...
1. **从图像检索候选位置。** 将 20 张参考照片与 30 个 M4 渲染视点编码为 DINOv2-small 特征，按 CLS 余弦相似度排序。候选视点按建图采样每六帧选取，检索只看图像。
...
1. **解析语言并选择实例。** 受限中文解析器提取类别、植物属性、盆体属性和方位，在 M4 生成的对象表中选择唯一匹配目标；有歧义时拒绝执行。控制器不读取评测器保存的目标答案。
...
<p class="table-heading" id="table-6-title"><span class="table-number">Table 6.</span> M4 场景中的下游任务结果与成功判据</p>
